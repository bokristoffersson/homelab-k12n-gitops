...
#[component]
pub fn HeatpumpStatusCard(data: Result<HeatpumpStatus, ApiError>) -> impl IntoView {
    match data {
        Ok(heatpump) => view! {
            <div class="card">
...
#[component]
fn StatusBadge(label: &'static str, value: Option<bool>) -> impl IntoView {
    let is_on = value.unwrap_or(false);
    let dot_class = if is_on {
        "status-dot on"
    } else {
        "status-dot off"
    };
