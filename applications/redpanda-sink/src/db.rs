...
pub async fn upsert_batch(pool: &DbPool, table: &str, upsert_key: &[String], rows: &[Row]) -> Result<(), AppError> {
    if rows.is_empty() { return Ok(()); }
    if upsert_key.is_empty() {
        return Err(AppError::Config("upsert_key cannot be empty for static data".into()));
    }

    let mut tag_cols: BTreeSet<&str> = BTreeSet::new();
    let mut field_cols: BTreeSet<&str> = BTreeSet::new();
    for r in rows { 
        for k in r.tags.keys() { tag_cols.insert(k.as_str()); } 
        for k in r.fields.keys() { field_cols.insert(k.as_str()); } 
    }
    ...
    for key_col in upsert_key {
        if !tag_cols.contains(key_col.as_str()) && !field_cols.contains(key_col.as_str()) {
            return Err(AppError::Config(format!("upsert_key column '{}' not found in tags or fields", key_col)));
        }
    }
