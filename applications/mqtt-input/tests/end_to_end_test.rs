...
async fn test_mqtt_to_redpanda_flow() {
    ...
    fields.insert("temperature".to_string(), FieldConfig {
    ...
    let result = ingestor.handle_message(
        &pipelines,
        "test/telemetry",
    ...
    consumer.subscribe(&["test-output"]).expect("Failed to subscribe");
    ...
    if let Ok(Ok(message)) = message_result {
        ...
        assert_eq!(tags.get("device_id").unwrap().as_str().unwrap(), "test-device");
...
async fn test_interval_throttling_e2e() {
    ...
    let result1 = ingestor.handle_message(
        &pipelines,
        "test/throttle",
        test_payload.to_string().as_bytes(),
    ).await;
    ...
    let result2 = ingestor.handle_message(
        &pipelines,
        "test/throttle",
        test_payload.to_string().as_bytes(),
    ).await;
