...
pub fn build_options(
    ...
    let client_id = format!("mqtt-to-timescale-{}", Uuid::new_v4());
    ...
    if let (Some(u), Some(p)) = (username, password) { opts.set_credentials(u.clone(), p.clone()); }
    if port == 8883 { opts.set_transport(Transport::tls_with_default_config()); }
    Ok(opts)
}

pub fn new(options: MqttOptions) -> (AsyncClient, EventLoop) { mqtt5::AsyncClient::new(options, 50) }
