...
The Redpanda Connect instance for heatpump-settings acts as a message processor and gateway between the Kafka streaming platform and the heat pump control system.

## Configuration

### Input

Consumes from the heatpump settings topic:
...
input:
  kafka:
    ...
    consumer_group: heatpump-settings
```
...
kubectl exec -n redpanda-v2 redpanda-v2-0 -- \
  rpk group describe heatpump-settings
```
...
2. Verify heat pump control is accessible:
   ```bash
   kubectl exec -n heatpump-settings deployment/redpanda-connect -- \
     curl http://heatpump-control:8080/health
   ```

3. Check for validation errors in logs:
   ```bash
   kubectl logs -n heatpump-settings deployment/redpanda-connect | grep ERROR
   ```
...
1. Check Redpanda Connect resource usage:
   ```bash
   kubectl top pod -n heatpump-settings
   ```

2. Scale if needed:
   ```bash
   kubectl scale -n heatpump-settings deployment/redpanda-connect --replicas=2
   ```

## Deployment

The service is deployed in the `heatpump-settings` namespace:

```bash
kubectl get all -n heatpump-settings
```

### Restart

```bash
kubectl rollout restart -n heatpump-settings deployment/redpanda-connect
```
